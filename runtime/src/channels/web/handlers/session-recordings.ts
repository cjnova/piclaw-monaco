...
function parseTrace(txt){ const trimmed=txt.trim(); if(!trimmed) return fixture; if(trimmed.startsWith('{')) return JSON.parse(trimmed); return {meta:{id:'dropped', title:'Dropped trace'}, events:trimmed.split(/\n+/).map(l=>JSON.parse(l))}; }
