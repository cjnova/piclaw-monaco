...
interface SettingsData {
  ...
  providers?: Provider[];
  /* toolsets */
  toolsets?: Array<{ name: string; description?: string; tools?: Array<{ name: string; kind?: string; weight?: string; description?: string; summary?: string }> }>;
}
...
function ToolsSection({ data }: { data: SettingsData }) {
  ...
  const filteredToolsets = filter.value
    ? toolsets.map(ts => ({
        ...ts,
        tools: (ts.tools ?? []).filter((t: any) =>
